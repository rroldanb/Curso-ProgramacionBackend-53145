...
const { deleteProductEmail } = require("../config/sendMail.config");
...
class ProductsController {
  ...
  createProduct = async (req, res, next) => {
    try {
      ...
      for (const campo of camposObligatorios) {
        if (!nuevoProducto[campo]) {
          CustomError.createError({
            ...
            message:
              "Falta uno de los campos obligatorios para la creación del producto",
      ...
      res
        .status(201)
        .json({ message: "Producto agregado correctamente", payload: result });
    } catch (error) {
